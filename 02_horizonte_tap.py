...
def rodar_ano(circuit, fator: float, comandos: list[str]) -> float:
    ...
    for h in range(24):
        ...
        if not circuit.Solution.Converged:
            ...
            raise RuntimeError(f"FALHA DE CONVERGÊNCIA: Hora {h}, Fator {fator}, Comandos {comandos}")
...
def main():
    ...
    for deriv, tap in derivacoes.items():
        ...
        for ano in range(1, ANOS_MAX + 1):
            ...
            cmds = [f"Edit Transformer.{TRAFO_ALVO} wdg=1 tap={tap}"] if deriv > 0 else [f"Edit Transformer.{TRAFO_ALVO} wdg=1 tap=1.0"] 
