...
print(
    f"  {'Linha':<25} {'Q médio':>10} {'Q máx':>10} {'P máx':>10} {'Bus1':<15} {'Bus2':<15}"
)
print(f"  {'-' * 90}")
...
for nome, q_med in sorted(q_medio_linha.items(), key=lambda x: x[1], reverse=True):
    if q_med > 10 and nome in linha_info:
        ...
        print(
            f"  {nome:<25} {q_med:>10.1f} {q_max_linha[nome]:>10.1f} {p_max_linha[nome]:>10.1f} {info['bus1']:<15} {info['bus2']:<15}"
        )
...
if not candidatos:
    ...
    for nome, q_med in sorted(q_medio_linha.items(), key=lambda x: x[1], reverse=True)[
        ...
        if nome in linha_info:
            info = linha_info[nome]
            print(
                f"  {nome:<25} Q_med={q_med:>8.2f} Q_max={q_max_linha[nome]:>8.2f} kvar"
            )
...
while idx > 0:
    if "segmmt" in meters.Name.lower() or idx == 1:
        ...
        if len(regs) > 1:
            ...
            e_kvarh = regs[1]
            fp_medio = (
                e_kwh / (e_kwh**2 + e_kvarh**2) ** 0.5
                if (e_kwh**2 + e_kvarh**2) > 0
                else 0
            )
            print(f"\nMedidor: {meters.Name}")
            ...
            if fp_medio > 0.95:
                print(
                    f"  >>> FP > 0,95 — rede bem compensada, capacitor não indicado globalmente"
                )
            elif fp_medio > 0.92:
                print(
                    f"  >>> FP entre 0,92 e 0,95 — compensação marginal pode ser benéfica"
                )
            else:
...
if candidatos:
    ...
    print(
        f"  Capacitor sugerido: {int(melhor['q_medio'] * 0.8 / 100) * 100} kvar (80% da demanda média)"
    )
else:
