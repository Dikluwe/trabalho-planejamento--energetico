...
print(
    f"  Tensão média  : {sum(v[0] for v in mt_buses.values()) / len(mt_buses):.4f} pu"
)
...
print(
    f"  Total em faixa precária ou crítica: {len(criticos_bt)} de {len(bt_buses)} barramentos BT"
)
...
print(
    f"  Tensão média  : {sum(v[0] for v in bt_buses.values()) / len(bt_buses):.4f} pu"
)
...
for ano, mult in [
    ...
    dss.Text.Command = (
        f"Set LoadMult={mult}"  # Altera o fator de carga direto na memória
    )
    trafo_max = {}
    circuit.Solution.dblHour = 0.0
    for h in range(24):
...
for nome, anos in resultado_trafos.items():
    ...
    if max(a1, a2, a3) > 80:
        if a1 > 100:
            status = "SOBRECARGA Ano1"
        elif a2 > 100:
            status = "SOBRECARGA Ano2"
        elif a3 > 100:
            status = "SOBRECARGA Ano3"
        elif a3 > 80:
            status = "alerta Ano3"
        elif a2 > 80:
            status = "alerta Ano2"
...
for h in range(24):
    ...
    delta_total += delta
    print(
        f"  {h + 1:>4} {h_perdas_com[h]:>10.2f} {h_perdas_sem[h]:>10.2f} {delta:>8.2f} {h_vmin_com[h]:>10.4f} {h_vmin_sem[h]:>10.4f}"
    )
