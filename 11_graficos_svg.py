...
for ano in [1, 2, 3]:
    ...
    if ano == 1:
        ...
        perfil_carga = [
            s + g - p for s, g, p in zip(perfil_sub, perfil_gd, perfil_perd)
        ]
...
print("\nGerando gráficos...")
ls = svg_base(
    f"Carregamento do Transformador {TRAFO_ALVO}", "Perfil diário — Anos 1, 2 e 3"
)
...
for ano, cor in [(1, CORES["ano1"]), (2, CORES["ano2"]), (3, CORES["ano3"])]:
    pts = " ".join(
        f"{esc(h, 1, 24):.1f},{esc_y(v, ymin, ymax):.1f}"
...
# Legenda
for i, (ano, cor, label) in enumerate(
    [
        (1, CORES["ano1"], "Ano 1 (×1,0)"),
    ...
    ls.append(
        f'<text x="{lx + 22}" y="{ly + 3}" {FONT} font-size="8" '
...
# ===========================================================================
ls = svg_base("Balanço Energético — Ano 1", "Subestação · GD Solar · Perdas")
...
ls.append(
    f'<polygon points="{pts_carga} {pts_base}" '
    f'fill="{CORES["carga"]}" fill-opacity="0.15" stroke="none"/>'
)
ls.append(
    f'<polyline points="{pts_carga}" fill="none" '
    f'stroke="{CORES["carga"]}" stroke-width="1.5"/>'
)

# Linha subestação
pts_sub = " ".join(
    f"{esc(h, 1, 24):.1f},{esc_y(v, ymin, ymax):.1f}" for h, v in zip(horas, perfil_sub)
)
ls.append(
    f'<polyline points="{pts_sub}" fill="none" '
    f'stroke="{CORES["sub"]}" stroke-width="1.5"/>'
)

# Linha GD
pts_gd = " ".join(
    f"{esc(h, 1, 24):.1f},{esc_y(v, ymin, ymax):.1f}" for h, v in zip(horas, perfil_gd)
)
ls.append(
    f'<polyline points="{pts_gd}" fill="none" '
...
def faixa(ls, y1, y2, cor, alpha, label=""):
    ...
    h_px = abs(yp1 - yp2)
    ls.append(
        f'<rect x="{PAD["left"]}" y="{min(yp1, yp2):.1f}" '
        f'width="{PW}" height="{h_px:.1f}" '
        f'fill="{cor}" fill-opacity="{alpha}" stroke="none"/>'
    )
    if label:
        ls.append(
            f'<text x="{PAD["left"] + PW - 2}" y="{min(yp1, yp2) + 10:.1f}" '
...
ls.append(
    f'<polygon points="{pts_max} {pts_min_rev}" '
...
ls.append(
    f'<polyline points="{pts_vmax}" fill="none" '
    f'stroke="{CORES["ano3"]}" stroke-width="1.2"/>'
)
ls.append(
    f'<polyline points="{pts_vmin}" fill="none" '
...
# Legenda
ls.append(
    f'<line x1="{PAD["left"] + 5}" y1="{PAD["top"] + 9}" '
    f'x2="{PAD["left"] + 23}" y2="{PAD["top"] + 9}" '
    f'stroke="{CORES["ano3"]}" stroke-width="1.2"/>'
)
ls.append(
    f'<text x="{PAD["left"] + 27}" y="{PAD["top"] + 12}" '
    f'{FONT} font-size="8" fill="{CORES["texto"]}">Vmax</text>'
)
ls.append(
    f'<line x1="{PAD["left"] + 65}" y1="{PAD["top"] + 9}" '
    f'x2="{PAD["left"] + 83}" y2="{PAD["top"] + 9}" '
    f'stroke="{CORES["sub"]}" stroke-width="1.2"/>'
)
ls.append(
    f'<text x="{PAD["left"] + 87}" y="{PAD["top"] + 12}" '
...
# ===========================================================================
taxas = [0.06, 0.08, 0.10, 0.12, 0.14, 0.16, 0.18, 0.20, 0.25, 0.30, 0.40, 0.50, 0.54]
...
ymax_v = max(vpls) + 100
grade_h(
    ls, [v for v in range(-500, 2500, 500) if ymin_v <= v <= ymax_v], ymin_v, ymax_v
)

# Linha VPL=0
y0 = esc_y(0, ymin_v, ymax_v)
ls.append(
    f'<line x1="{PAD["left"]}" y1="{y0:.1f}" '
...
ls.append(
    f'<circle cx="{xtir:.1f}" cy="{y0:.1f}" r="4" '
    f'fill="{CORES["ano3"]}" stroke="white" stroke-width="1"/>'
)
ls.append(
    f'<text x="{xtir + 6:.1f}" y="{y0 - 5:.1f}" '
    f'{FONT} font-size="8" fill="{CORES["ano3"]}">TIR≈54%</text>'
)

# Curva VPL
pts = " ".join(
    f"{esc(t * 100, taxas[0] * 100, taxas[-1] * 100):.1f},{esc_y(v, ymin_v, ymax_v):.1f}"
    for t, v in zip(taxas, vpls)
)
ls.append(
    f'<polyline points="{pts}" fill="none" stroke="{CORES["tap"]}" stroke-width="2"/>'
)
...
ls.append(
    f'<circle cx="{x14:.1f}" cy="{y14:.1f}" r="4" '
    f'fill="{CORES["tap"]}" stroke="white" stroke-width="1"/>'
)
ls.append(
    f'<text x="{x14 + 5:.1f}" y="{y14 - 5:.1f}" '
...
eixo_x_labels(
    ls,
    [
        esc(t * 100, taxas[0] * 100, taxas[-1] * 100)
        for t in [0.06, 0.10, 0.14, 0.20, 0.30, 0.40, 0.54]
    ],
    ["6%", "10%", "14%", "20%", "30%", "40%", "54%"],
    PAD["top"] + PH,
)
...
ls = svg_base(
    "Curva de Duração de Carga — trf_6\u20134910a",
...
for gi, (limiar, vals) in enumerate(duracao.items()):
    ...
    ls.append(
        f'<text x="{lx:.1f}" y="{PAD["top"] + PH + 10}" '
    ...
    for ai, (v, cor) in enumerate(zip(vals, cores_anos)):
        ...
        by = PAD["top"] + PH - bh
        ls.append(
            f'<rect x="{bx:.1f}" y="{by:.1f}" width="{bw * 0.85:.1f}" '
            f'height="{bh:.1f}" fill="{cor}" fill-opacity="0.85" rx="1"/>'
        )
        if v > 0:
            ls.append(
                f'<text x="{bx + bw * 0.42:.1f}" y="{by - 2:.1f}" '
...
for i, (cor, label) in enumerate(zip(cores_anos, ["Ano 1", "Ano 2", "Ano 3"])):
    ...
    ly = PAD["top"] + 8
    ls.append(
        f'<rect x="{lx}" y="{ly - 6}" width="14" height="8" '
        f'fill="{cor}" fill-opacity="0.85" rx="1"/>'
    )
    ls.append(
        f'<text x="{lx + 18}" y="{ly + 1}" {FONT} font-size="8" '
        f'fill="{CORES["texto"]}">{label}</text>'
    )
...
def escx(v):
    return PAD["left"] + (v - xmin_r) / (xmax_r - xmin_r) * PW2
...
for xv in [0.980, 0.990, 1.000, 1.010, 1.020, 1.030, 1.040, 1.050, 1.060, 1.070, 1.080]:
    ...
    ls.append(
        f'<text x="{xp:.1f}" y="{PAD["top"] + PH2 + 10}" '
