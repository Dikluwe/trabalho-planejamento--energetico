...
class DailyNetworkAnalyzer:
    def __init__(self, simulador, total_hours: int, low_voltage_kv: float, lower_v_pu: float, upper_v_pu: float, output_folder: str):
    ...
    def run_analysis_and_export(self) -> dict:
        ...
        for hour in range(self.total_hours):
            ...
            all_voltages.extend(self.extrator.get_voltages(hour, bus_data, load_bus_set, self.lower_v_pu, self.upper_v_pu))
            all_lines.extend(self.extrator.get_lines(hour))
            all_transformers.extend(self.extrator.get_transformers(hour))

            current_meters, previous_meter_state = self._process_meter_step(hour, previous_meter_state)
        ...
        df_network_summary = self._build_network_summary(
            df_voltage_summary, df_line_summary, df_transformer_summary, df_energy_summary
        )

        # Exportação para CSV
        self._export_to_csv(
            df_voltages, df_lines, df_transformers, df_meter_by_hour,
            df_voltage_summary, df_line_summary, df_transformer_summary,
            df_meter_hour_summary, df_energy_summary, df_network_summary
        )
    ...
    def _process_meter_step(self, hour: int, previous_state: dict):
        ...
        for row in current_state_list:
            ...
            prev_row = previous_state.get(meter_name, {
                "activeEnergyKWh": 0.0, "reactiveEnergyKvarh": 0.0,
                "activeLossesKWh": 0.0, "reactiveLossesKvarh": 0.0
            })
            ...
            rows.append({
                ...
                "deltaActiveEnergyKWh": row["activeEnergyKWh"] - prev_row["activeEnergyKWh"],
                "deltaReactiveEnergyKvarh": row["reactiveEnergyKvarh"] - prev_row["reactiveEnergyKvarh"],
                "deltaActiveLossesKWh": row["activeLossesKWh"] - prev_row["activeLossesKWh"],
                "deltaReactiveLossesKvarh": row["reactiveLossesKvarh"] - prev_row["reactiveLossesKvarh"]
            })
    ...
    def _build_network_summary(self, df_voltage, df_line, df_transformer, df_energy) -> pd.DataFrame:
        mv_pct = 0.0
        lv_pct = 0.0
        ...
        if not df_voltage.empty:
            ...
            if not mv_data.empty: mv_pct = mv_data.iloc[0]["violationPct"]
            if not lv_data.empty: lv_pct = lv_data.iloc[0]["violationPct"]

        ovl_lines = int((df_line["maxLoadingPct"] > 100.0).sum()) if not df_line.empty else 0
        ovl_trafo = int((df_transformer["maxLoadingPct"] > 100.0).sum()) if not df_transformer.empty else 0
        
        max_line_pct = df_line["maxLoadingPct"].max() if not df_line.empty else 0.0
        max_trafo_pct = df_transformer["maxLoadingPct"].max() if not df_transformer.empty else 0.0
    ...
    def _export_to_csv(self, *dfs):
        ...
        filenames = [
            "VoltagesByHour.csv", "LinesLoadingByHour.csv", "TransformersLoadingByHour.csv",
            "EnergyMetersByHour.csv", "VoltageSummary.csv", "LineSummary.csv",
            "TransformerSummary.csv", "EnergyMeterHourSummary.csv", "EnergySummary.csv",
