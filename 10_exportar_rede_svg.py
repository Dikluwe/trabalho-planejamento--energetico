...
def projeto(lon, lat, xmin, ymin, xmax, ymax, W, H, margin=20):
    ...
    dx = xmax - xmin
    dy = ymax - ymin
    scale = min((W - 2*margin) / dx, (H - 2*margin) / dy) if dx > 0 and dy > 0 else 1
...
def camada_trafos(p):
    els = ['  <!-- Transformadores -->',
           '  <g id="transformadores">']
    for t in trafos:
        cx, cy = p(t["pos"][0], t["pos"][1])
        r      = max(2, min(8, t["kva"] / 30))  # raio proporcional ao kVA
        cor    = "#e74c3c" if t["pct"] > 100 else ("#e67e22" if t["pct"] > 80 else "#2ecc71")
        tip    = f'{t["nome"]} | {t["kva"]:.0f} kVA | {t["pct"]:.1f}%'
        els.append(f'    <circle cx="{cx:.1f}" cy="{cy:.1f}" r="{r:.1f}" '
...
def camada_gd(p):
    els = ['  <!-- PVSystems (GD Fotovoltaica) -->',
           '  <g id="gd_fotovoltaica">']
    for pv in pvs:
...
def camada_problemas(p):
    ...
    for t in trafos:
        if t["nome"].lower() in TRAFOS_SOBRECARGA:
            ...
            els.append(f'    <line x1="{cx-s:.1f}" y1="{cy-s:.1f}" '
...
# Rede completa
svgs.append(gerar_svg("rede_completa.svg", [
    camada_linhas_bt,
    camada_linhas_mt,
    camada_trafos,
    camada_gd,
...
# Só problemas
svgs.append(gerar_svg("rede_problemas.svg", [
    camada_linhas_mt,
    camada_problemas,
]))
