...
resultados_reg = {}
for ano, mult, gd_f in [(1,1.0,1.0),(2,1.1,1.0-DEGRADACAO_GD),(3,1.2,1.0-2*DEGRADACAO_GD)]:
    # Sem regulador
    c = carregar(mult)
    if gd_f != 1.0:
    ...
    print(f"  {ano:>4} {'base':>12} {m_base['perdas_kwh']:>11.1f} "
          f"{m_base['vmin_bt']:>9.4f} "
          f"{m_base['trafo_max'].get('trf_6_4910a',0):>10.1f}")
    print(f"  {'':>4} {'c/regulador':>12} {m_reg['perdas_kwh']:>11.1f} "
...
cargas_ramal_4910 = cargas_por_bus.get("uc632607", []) + \
...
for desc, cargas_especiais, mult_geral in cenarios:
    ...
    print(f"  {desc:<35} {pct_4910:>10.1f} {m['perdas_kwh']:>11.1f} {m['vmin_bt']:>9.4f}")
