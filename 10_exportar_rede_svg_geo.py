...
def linhas_mt_svg():
    ...
    for (c1, c2, is_sw) in linhas_mt:
        ...
        els.append(f'    <line x1="{x1:.8f}" y1="{y1:.8f}" '
                   f'x2="{x2:.8f}" y2="{y2:.8f}" '
...
def trafos_svg():
    ...
    for t in trafos:
        ...
        cor = "#e74c3c" if t["pct"] > 100 else ("#e67e22" if t["pct"] > 80 else "#27ae60")
        tip = f'{t["nome"]} | {t["kva"]:.0f} kVA | {t["pct"]:.1f}%'
        els.append(f'    <circle cx="{cx:.8f}" cy="{cy:.8f}" r="{r:.8f}" '
...
def problemas_svg():
    ...
    for t in trafos:
        if t["nome"].lower() in TRAFOS_SOBRECARGA:
            ...
            els.append(f'    <line x1="{cx-s:.8f}" y1="{cy-s:.8f}" '
...
svgs_gerados = []
svgs_gerados.append(escrever_svg("rede_completa_geo.svg",
    [linhas_bt_svg, linhas_mt_svg, trafos_svg, gd_svg, problemas_svg],
    "CRELUZ — Alimentador 1_REDE2_1 — Rede Completa"))

svgs_gerados.append(escrever_svg("rede_linhas_mt_geo.svg",
    [linhas_mt_svg],
    "CRELUZ — Linhas MT"))

svgs_gerados.append(escrever_svg("rede_linhas_bt_geo.svg",
    [linhas_bt_svg],
    "CRELUZ — Linhas BT"))

svgs_gerados.append(escrever_svg("rede_trafos_geo.svg",
    [trafos_svg],
    "CRELUZ — Transformadores"))

svgs_gerados.append(escrever_svg("rede_gd_geo.svg",
    [gd_svg],
    "CRELUZ — GD Fotovoltaica"))

svgs_gerados.append(escrever_svg("rede_problemas_geo.svg",
